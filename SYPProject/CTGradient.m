...
+ (id)hydrogenSpectrumGradient
{
    ...
    struct {float hue; float position; float width;}
    colorBands[4];
...
- (void)setBlendingMode:(CTGradientBlendingMode)mode;
{
    ...
    if(gradientFunction != NULL)
        CGFunctionRelease(gradientFunction);
                                                  ...
                                                  //Version, evaluator function, cleanup function
    CGFunctionCallbacks evaluationCallbackInfo = {
        0 , evaluationFunction, NULL
    };

                                                  //range  for the evaluator input
    static const float input_value_range   [2] = {
        0, 1
    };
                                                  //ranges for the evaluator output (4 returned values)
    static const float output_value_ranges [8] = {
        0, 1, 0, 1, 0, 1, 0, 1
    };

                                                  //the two transition colors
    gradientFunction = CGFunctionCreate(&elementList,
