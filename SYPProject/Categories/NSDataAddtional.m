...
- (NSData *)AES128DecryptWithKey:(NSString *)key {
	...
	//bzero(keyPtr, sizeof(keyPtr)); // fill with zeroes (for padding)
	NSString *tempS = key;
	//NSString *tempS = @"pjyBIYZG6THGxfdQg0+mOw==";
	NSData *tempD = [tempS dataUsingEncoding:NSUTF8StringEncoding];
	NSData *temp64 = [GTMBase64 decodeData:tempD];
	//NSString *tempSt = [[NSString alloc] initWithData:temp64 encoding:NSUTF8StringEncoding];
	//const keyPtr = [temp64 bytes];
	...
	CCCryptorStatus cryptStatus = CCCrypt(kCCDecrypt, kCCAlgorithmAES128, kCCOptionPKCS7Padding,
										  [temp64 bytes]/*keyPtr*/, kCCKeySizeAES128, // oorspronkelijk 256
										  NULL /* initialization vector (optional) */,
										  [self bytes], dataLength, /* input */
										  buffer, bufferSize, /* output */
										  &numBytesDecrypted);
...
- (NSString*) base64Encode
{
	...
	NSString* ret = [[[NSString alloc] initWithData:[NSData dataWithBytes:buf length:bufLength] encoding:NSUTF8StringEncoding] autorelease];
