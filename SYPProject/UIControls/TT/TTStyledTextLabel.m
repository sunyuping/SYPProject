...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (!tableView) {
        if (_highlightedNode) {
            ...
            if ([_highlightedNode isKindOfClass:[TTStyledLinkNode class]]) {;//TTOpenURL([(TTStyledLinkNode*)_highlightedNode URL]);
                NSString *url = [(TTStyledLinkNode *) _highlightedNode URL];
//        RSWebController *webCtl  = [[[RSWebController alloc] initWithURL:[NSURL URLWithString:url]] autorelease];
//        [[RSNavigator navigator] openPushViewController:webCtl];
                if (url && self.openUrlBlock)
                    self.openUrlBlock(url);

            } else if ([_highlightedNode isKindOfClass:[TTStyledButtonNode class]]) {;//TTOpenURL([(TTStyledButtonNode*)_highlightedNode URL]);
                NSString *url = [(TTStyledLinkNode *) _highlightedNode URL];
//                RSWebController *webCtl = [[[RSWebController alloc] initWithURL:[NSURL URLWithString:url]] autorelease];
//                [[RSNavigator navigator] openPushViewController:webCtl];
                if (url && self.openUrlBlock)
...
- (void)drawRect:(CGRect)rect {
    if (_highlighted) {
        ...
        //const CGFloat *colorComs = CGColorGetComponents([[self textColor] CGColor]);
        //NSLog(@"styledText %f %f %f", colorComs[0], colorComs[1], colorComs[2]);
...
//    CGContextSetShadowWithColor(context, CGSizeMake(0, 3), 0, 
//    [[UIColor colorWithRed:1.0 green:0 blue:1.0 alpha:1.0] CGColor]);
